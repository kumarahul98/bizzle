...
- **Deletion model — soft for Trash, hard for erasure.** Per-trip trash (`DELETE /trips/:tripId`) is SOFT: it marks `deleted: true`, and Trash restore depends on that flag — do not make it hard. Bulk "delete all my data, keep my account" (`DELETE /trips`) is HARD: it erases every trip document for that uid, including trips already sitting in Trash, because the in-app dialog promises permanent deletion and the local Drift wipe is a genuine hard delete — the backend must match. Full account deletion (`DELETE /account`, Phase 38) is also HARD, for the same reason plus the uid can never authenticate again. Do not "fix" the bulk delete-all-data endpoint or `delete-account.ts` back to soft-delete — both are explicit, scoped product decisions.
