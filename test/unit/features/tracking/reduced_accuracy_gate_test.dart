...
void main() {
  group('IOS-08 reduced-accuracy gate — LocationAccuracyGate.ensurePrecise()',
      () {
    group('Outcome A: reduced → request → still reduced → BLOCKED', () {
      test('ensurePrecise() returns false', () async {
        ...
        expect(requestCalled, isTrue, reason: 'request must have been called');
        expect(
          capturedPurposeKey,
      ...
      test(
        ...
        () async {
          ...
          expect(
            requestCalled,
            isFalse,
            reason: 'requestTemporaryFullAccuracy must NOT be called when '
                'already precise',
          );
        },
      );
