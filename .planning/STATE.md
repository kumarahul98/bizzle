...
| 260805-wkk | Dashboard idle hero card rendered a hardcoded "To office" at every hour — `autoLabelDirection`/`autoLabelTime` were optional and no call site ever passed them, so the row always fell through to its literal default (observed on device at 22:58). `HeroRecordCard` now resolves the label itself from the user's cutoff prefs + local `DateTime.now()` via `DirectionLabelService`, mapped through `kDirectionToHomeLabel`/`kDirectionToOfficeLabel`. Both dead params deleted, along with `_AutoLabelRow.time`; `'Auto-labelled '` extracted to `kAutoLabelledPrefix`. Resolved in `build` rather than a `Provider` on purpose — a `Provider` caches until a dependency changes, so an open session would keep showing a label computed hours earlier. New widget test is clock-independent (evening cutoff 0 ⇒ always to_home; morning cutoff 24 ⇒ always to_office) and was adversarially verified to fail against the pre-fix code. Live recording header, notification, and persisted `trips.direction` were always correct and are untouched. Found by the pre-release device smoke test. | 2026-08-05 | b56b6b2 | [260805-wkk-fix-dashboard-idle-auto-label-direction](./quick/260805-wkk-fix-dashboard-idle-auto-label-direction/) |
| 260805-wvs | Three more user-facing defects from the same smoke test, one commit each. (1) `7b9fec5` — the delete-trip dialog claimed "This trip will be permanently removed" for what is a SOFT delete to Trash; copy now names the Trash and its retention window, composed around `kTrashRetentionDays`, with tests pinning it apart from the genuinely-irreversible `kTrashPermanentDeleteDialogBody`. Deletion wording is what Play's Data Safety review reads. (2) `9bc0050` — `insertManualTrip` omitted `directionSource`, so user-chosen directions were stored as `'time'`, making them eligible for `restore_controller`'s enrichment to overwrite the user's own pick (T-21-03-01 violation); now records `kDirectionSourceManual`, adversarially verified. (3) `6ad58f7` — the Trips toolbar icon buttons had no semantics at all and announced nothing to screen readers; `semanticLabel` is now required, and the view toggle's label names the view a tap will open. | 2026-08-05 | 7b9fec5, 9bc0050, 6ad58f7 | [260805-wvs-fix-three-user-facing-smoke-test-defects](./quick/260805-wvs-fix-three-user-facing-smoke-test-defects/) |
| 260806-0t6 | Stuck segments now sync to the cloud. `trip_stuck_segments` was device-only — `TripDoc` had `breaks` but no segment field — so a reinstall restored a trip's aggregate `timeStuckSeconds` while silently dropping every painted slow stretch on its map (confirmed on device: two restored trips, 1880s and 3501s stuck, zero segment rows). Mirrors the Phase 26 breaks path end to end: `StuckSegment` type, zod schema with `kMaxStuckSegmentsPerTrip = 200` (4x the breaks cap — segments are emitted automatically per slow run, not per user pause), converter defaulting for legacy docs, two new batched DAO reads, `ParsedTrip` extension, transactional restore insert, and enrichment for trips already present locally. Merge deliberately does NOT copy the breaks ride-along rule: segment indices address the decoded polyline and the merged trip always takes cloud's, so cloud segments win when present and local ones only when the geometry is identical — which also fixed a latent bug in the conflict sheet's "Use Cloud" branch that left local segments against a replaced polyline. Backward compatible both directions. **Deployed to traevy-prod**; `/health` 200, sync/restore 401, bogus 404. Flutter 1034 pass, backend 127/127. versionCode → 6. Device round-trip NOT yet verified. | 2026-08-06 | 30e85ad | [260806-0t6-sync-stuck-segments-to-cloud](./quick/260806-0t6-sync-stuck-segments-to-cloud/) |
